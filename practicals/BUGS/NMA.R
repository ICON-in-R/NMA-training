...
if (!require("BCEA")) install.packages("BCEA")
...
names(smoke.list)
lapply(smoke.list,class)  # format/type
...
res <- bugs(
  model=here::here("practicals","BUGS","smokefix_model.txt"), 
  data=smoke.list,
  inits=inits,
  parameters=c("d"),
  n.chains=2,
...
## How many further iterations do we need after convergence?
## Consider the "effective sample size".
## Rule of thumb is you need n.eff > 4000
## to ensure 95\% credible limits have 94.5-95.5% coverage (true in this case).  
...
res2 <- bugs(
  model=here::here("practicals", "BUGS","smokere_model.txt"), 
  data=smoke.list,
  inits=inits,
...
## Appendix:
## Original pre-processing of dataset to format the data in a suitable way

smoke <- read.table(
  here::here("practicals","BUGS","smoke_data_orig.txt"), 
  header=TRUE, nrow=24)
